...
from src.model.photomaker.attn_processor import AttnProcessor2_0 as AttnProcessor
from src.model.photomaker.attn_processor import IPAttnProcessorFace
...
class PhotoMakerStableDiffusionXLIPPipeline(StableDiffusionXLPipeline):
    ...
    def set_ip_adapter(self):
        ...
        for name in self.unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                if self.attn_type == "face_other":
                    attn_procs[name] = IPAttnProcessorFace(
    ...
    def __call__(
        self,
        prompt_face: Union[str, List[str]] = None,
        prompt_other: Union[str, List[str]] = None,
        ...
        **kwargs,
        ...
        assert isinstance(prompt_face, str)
        batch_size = 1

        device = self._execution_device

        # 3.0 Encode other prompt

        (
            other_prompt_embeds, 
            _,
            other_pooled_prompt_embeds,
            _,
            other_class_tokens_mask,
        ) = self.encode_prompt(
            prompt=prompt_other,
            prompt_2=prompt_2,
            device=device,
        ...
        tokens_text_only = self.tokenizer.encode(prompt_face, add_special_tokens=False)
        ...
        print("other_prompt_embeds", other_prompt_embeds.shape)

        prompt_embeds_text_only = torch.cat([prompt_embeds_text_only, other_prompt_embeds], dim=1)        
        prompt_embeds = torch.cat([prompt_embeds, other_prompt_embeds], dim=1)
