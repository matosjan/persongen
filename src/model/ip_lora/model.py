...
from src.model.ip_lora.orig_id_encoder import OrigPhotoMakerIDEncoder
...
from src.model.attn_procs.attn_processor import myLoRAIPAttnProcessor2_0 as LoRAIPAttnProcessor 
...
class IPMakerLora(nn.Module):
    def __init__(self, pretrained_model_name_or_path, weight_dtype, rank, device='cuda', trigger_word='img'):
        super().__init__()
    ...
    def prepare_for_training(self):
        ...
        for name, module in self.unet.named_modules():
            ...
            if "lora" in name.lower() and 'to_k_ip' not in name.lower() and 'to_v_ip' not in name.lower():
                loras.append(module)
        self.lora_modules = nn.ModuleList(loras)
        print(self.unet)
        
        self.load_state_dict_({'lora_weights': torch.load('photomaker-v1.bin', map_location=self.device)['lora_weights']})
        ...
        for name in self.unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                weights = {
                    ...
                    "to_k_lora.down.weight": unet_sd[layer_name + ".to_k.lora_A.default.weight"],
                    "to_v_lora.down.weight": unet_sd[layer_name + ".to_v.lora_A.default.weight"],
                    "to_k_lora.up.weight": unet_sd[layer_name + ".to_k.lora_B.default.weight"],
                    "to_v_lora.up.weight": unet_sd[layer_name + ".to_v.lora_B.default.weight"]
                }
                attn_procs[name] = LoRAIPAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, num_tokens=self.num_text_tokens, rank=self.lora_rank, network_alpha=self.lora_rank)
                attn_procs[name].load_state_dict(weights)
        self.unet.set_attn_processor(attn_procs)
        self.adapter_modules = torch.nn.ModuleList(self.unet.attn_processors.values())
        
        # print(self.adapter_modules.state_dict().keys())
    ...
    def load_state_dict_(self, state_dict):
        ...
        if 'adapter_modules' in state_dict.keys():
            ...
            print(state_dict["adapter_modules"].keys())
        ...
        if 'lora_weights' in state_dict.keys():
            orig_unet_weights_sum = torch.sum(torch.stack([torch.sum(p) for p in self.lora_modules.parameters()]))
            ...
            if incompatible_keys is not None:
                ...
                if unexpected_keys:
                    print(
                        f"Loading lora_weights from state_dict led to unexpected keys not found in the model: "
                        f" {unexpected_keys}. "
                    )
                    assert 1 == 0            
            new_unet_weights_sum = torch.sum(torch.stack([torch.sum(p) for p in self.lora_modules.parameters()]))

            # assert orig_unet_weights_sum != new_unet_weights_sum, "Weights of unet did not change (model)"
            print('Loaded lora_weights into model')
        else:
            print('No lora_weights in state_dict')
