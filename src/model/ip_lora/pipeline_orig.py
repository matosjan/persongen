...
class IPMakerLoraStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def load_photomaker_adapter(
        ...
        **kwargs,
        ...
        self.id_encoder = self.id_encoder.to(self.device, dtype=self.unet.dtype)  

        new_encoder_sum = torch.sum(torch.stack([torch.sum(p) for p in self.id_encoder.parameters()]))

        assert orig_encoder_sum != new_encoder_sum, 'Weights of id_encoder did not change (pipe)'
        print('Loaded id_encoder weights into pipeline')
        ...
        assert orig_adapter_sum != new_adapter_sum, 'Weights of adapter modules did not change (pipe)'
        print('Loaded adapter_modules into pipeline')
    ...
    def __call__(
        ...
        **kwargs,
        ...
        (
            prompt_embeds,
            negative_prompt_embeds,
            pooled_prompt_embeds, # TODO: replace the pooled_prompt_embeds with text only prompt
            negative_pooled_prompt_embeds,
        ) = self.encode_prompt(
        ...
        ip_embeds = self.id_encoder(id_pixel_values)
        uncond_ip_embeds = torch.zeros_like(ip_embeds)
    
        bs_embed, num_inputs, _, _ = ip_embeds.shape
        ip_embeds = ip_embeds.view(bs_embed, num_inputs, -1)
        uncond_ip_embeds = uncond_ip_embeds.view(bs_embed, num_inputs, -1)

        ip_embeds = ip_embeds.repeat(1, num_images_per_prompt, 1)
        ip_embeds = ip_embeds.view(bs_embed * num_images_per_prompt, num_inputs, -1)
        uncond_ip_embeds = uncond_ip_embeds.repeat(1, num_images_per_prompt, 1)
        uncond_ip_embeds = uncond_ip_embeds.view(bs_embed * num_images_per_prompt, num_inputs, -1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i <= start_merge_step:
                    ...
                    negative_prompt_embeds = torch.cat([negative_prompt_embeds, uncond_ip_embeds], dim=1)
                    current_prompt_embeds = torch.cat(
                        [negative_prompt_embeds, prompt_embeds], dim=0
                    ) if self.do_classifier_free_guidance else prompt_embeds
                    add_text_embeds = torch.cat(
