...
class Aligner():
    def __init__(self):
        self.face_detector = FaceAnalysis2(providers=['CPUExecutionProvider', 'CUDAExecutionProvider'], allowed_modules=['detection', 'recognition'])
    ...
    def __call__(self, input_id_images):
        ...
        for orig_img in input_id_images:
            ...
            print("before clip", orig_arr.shape, bbox)
            bbox = np.clip(bbox, 0, max(orig_img.size))
            print("after clip", orig_arr.shape, bbox)
